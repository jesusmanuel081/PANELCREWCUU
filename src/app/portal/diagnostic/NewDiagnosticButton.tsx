...
export default function NewDiagnosticButton() {
  ...
  const handleCreate = async () => {
    ...
    if (!location.trim()) {
      setError("La ubicación de los paneles es obligatoria.");
      return;
    }
    if (!panelCount || Number(panelCount) < 1) {
      setError("Ingresa la cantidad de paneles (mínimo 1).");
      return;
    }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            Ubicación de los paneles <span className="text-red-500">*</span>
          </label>
          <input
            type="text"
            value={location}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            Cantidad de paneles (aprox.) <span className="text-red-500">*</span>
          </label>
          <input
            type="number"
            value={panelCount}
